...
def aosring_sign(pkeys: list, mypair: list, tees=None, alpha=None, message=None):
    assert len(pkeys) > 0
...
if __name__ == "__main__":
    ...
    print(quotelist([item.n for sublist in proof[0] for item in sublist]) + ',' + quotelist(proof[1]) + ',' + quote(
        proof[2]) + ',' + quote(msg))
